     ...
     className="w-full flex xl:flex-row justify-center min-h-screen gap-10 max-container
      flex-col">
        ...
        max-xl:py-40 bg-primary bg-hero bg-cover bg-center">
          ...
          <div className="flex sm:gap-6 absolute gap-4 -bottom-[5%] sm:left-[10%] max-sm:px-6">
            {shoes.map((shoe) => (
              <div key={shoe}>
                <ShoeCard 
                imgURL = {shoe} changeBigShoeImage={(shoe)=>{setBigShoeImg(shoe)}} bigShoeImage={bigShoeImg}/>
              </div>
            ))}
          </div>
        </div>
